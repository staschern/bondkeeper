...
- До находки про `/iss/securities.json?q=`: 3002/3061 (98,1%) с `inn = NULL` у всех эмитентов, 59 ОФЗ пропущены.
- После переписывания на новый эндпоинт — ожидается закрытие обоих пробелов разом (реальный ИНН + все 59 ОФЗ); повторный прогон дозаполняет уже существующие записи (`ON DUPLICATE KEY UPDATE` теперь перезаписывает `inn`/имена, а не только `updated_at`).
