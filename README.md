...
**Итог боевого прогона** (`seed_market.php` на bondkeeper.ru, весь рынок 3061 бумага, после миграций 002+003): **3002 успешно (98,1%)**, стабильно воспроизводится между запусками. Остаток — 59 бумаг без `EMITTER_ID` в `description` (устойчивый список, не исследовано отдельно, что это за инструменты — вероятно нестандартные/структурные, не блокирует остальной импорт).
