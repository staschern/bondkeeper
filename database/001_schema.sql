...
--
-- v2.1 — два исправления, найденные при развёртывании на реальном MySQL
-- 8.0.46 (не выявлялись при обзорном разборе, только при факт. CREATE):
--   1. event_stories: убран CHECK chk_event_stories_exactly_one — MySQL 8
--      отклоняет CHECK на колонке, у которой есть FK с ON DELETE SET NULL
--      (ошибка 3823). Правило "ровно одно из trex полей" переносится в
--      код классификатора событий (единственное место INSERT в эту таблицу).
--   2. watchlist: FK на issuer_id/security_id вынесены в отдельный ALTER
--      TABLE, и ON DELETE CASCADE заменён на RESTRICT — MySQL 8 запрещает
--      CASCADE/SET NULL/SET DEFAULT на базовом столбце STORED-колонки
--      (issuer_only_key зависит от issuer_id и security_id), ошибка 3192.
--      См. MySQL Reference Manual §13.1.20.8.
-- Сама таксономия, поля и структура кластеров не менялись.
-- =====================================================================
...
CREATE TABLE event_stories (
    ...
    -- Правило "ровно одно из трёх полей заполнено" НЕЛЬЗЯ выразить через
    -- CHECK в MySQL 8: сервер отклоняет CHECK на колонке, у которой есть
    -- FK с ON DELETE SET NULL (ошибка 3823 — потому что удаление
    -- родительской строки может молча нарушить условие). Проверено на
    -- реальном MySQL 8.0.46. Правило переносится в код приложения
    -- (единственное место создания event_stories — классификатор событий).
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- FK-и на issuer_id/security_id вынесены в отдельный ALTER TABLE: MySQL 8
-- не может в ОДНОМ CREATE TABLE одновременно объявить STORED GENERATED
-- колонку, зависящую от столбца (issuer_only_key <- issuer_id/security_id),
-- и inline FOREIGN KEY на этот же столбец — падает с ошибкой 1215
-- независимо от FOREIGN_KEY_CHECKS. Проверено на реальном MySQL 8.0.46.
--
-- ON DELETE CASCADE у fk_watchlist_issuer/fk_watchlist_security заменён на
-- RESTRICT: issuer_id и security_id — базовые столбцы STORED-колонки
-- issuer_only_key, а MySQL 8 официально запрещает CASCADE/SET NULL/SET
-- DEFAULT в качестве referential action на базовом столбце сгенерированной
-- колонки (ошибка 3192, см. MySQL Reference Manual §13.1.20.8). На практике
-- issuers/securities не удаляются физически (см. status ENUM в securities
-- и is_fns_blocked-подход в issuers) — RESTRICT здесь ничего не меняет по
-- сути, просто явно запрещает удаление каталожной записи, пока на неё есть
-- ссылки в чьём-то watchlist.
ALTER TABLE watchlist
    ADD CONSTRAINT fk_watchlist_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    ADD CONSTRAINT fk_watchlist_issuer FOREIGN KEY (issuer_id) REFERENCES issuers(id) ON DELETE RESTRICT,
    ADD CONSTRAINT fk_watchlist_security FOREIGN KEY (security_id) REFERENCES securities(id) ON DELETE RESTRICT;
