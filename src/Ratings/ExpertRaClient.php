...
final class ExpertRaClient
{
    ...
    /**
     * ИНН рейтингуемого лица со страницы КОНКРЕТНОГО пресс-релиза (не
     * карточки компании — другая страница, другой блок). Найдено вживую
     * (сентябрь 2026, после того как первая версия ExpertRaNewsImporter
     * ошибочно считала, что Эксперт РА "никогда не даёт ИНН" — это было
     * верно только для ленты новостей/AJAX-чанка (title+subtitle), но
     * НЕ для полного текста самого пресс-релиза, который эта лента просто
     * не открывала): у каждого пресс-релиза есть раздел "Регуляторное
     * раскрытие" — обычная HTML-таблица (`<td>Идентификационный номер
     * налогоплательщика рейтингуемого лица</td><td>1234567890</td>`),
     * тот же принцип, что у НКР, но без вставки "(ИНН)" в скобках —
     * формулировка агентства чуть другая, общий regex НКР сюда не
     * подходит буквально.
     *
     * Пустой результат (`NULL`) — ожидаемо и не ошибка для двух реальных
     * случаев: объект рейтинга без ИНН юрлица (регионы/субъекты РФ — поле
     * прямо содержит текст "Отсутствует", проверено на реальном релизе
     * "Ростовская область") и, гипотетически, иностранные юрлица (по
     * аналогии с карточками компаний, где такое уже встречалось).
     */
    public function fetchReleaseInn(string $releaseUrl): ?string
    {
        $html = RatingsHttp::get($releaseUrl);
        $text = $this->toPlainText($html);

        if (preg_match('/Идентификационный\s+номер\s+налогоплательщика\s+рейтингуемого\s+лица\s+(\d{10,12})/u', $text, $m)) {
            return $m[1];
        }
