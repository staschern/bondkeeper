 ...
 * все 4 нет (АКРА пока не реализована — см. docs/STAGE3_RATINGS.md,
 * недоступна с этой машины из-за проблемы с TLS-сертификатом на их
 * сервере).
 *
 * Запуск:
 ...
 * ИНН — единственный надёжный способ сопоставить с issuers.id). Это
 * порядка 400-500 запросов за прогон — с задержкой между КАЖДЫМ запросом
 * (--delay-ms, по умолчанию 400мс), не единомоментный экспорт, поэтому
 * может идти несколько минут — это ожидаемо, не зависание.
 *
